...
public interface SheetRepository extends JpaRepository<SheetModel, UUID> {
    ...
    @Query(value = SHEET_PAGINATED_BASE_SELECT + " from SheetModel s where s.player.id = ?1 and s.isActive = true and lower(s.name) like lower(concat('%', ?2,'%'))",
            countQuery = "select count(*) from SheetModel s where s.player.id = ?1 and s.isActive = true and lower(s.name) like lower(concat('%', ?2,'%'))")
    Page<BasicSheetDto> findActivePlayerSheetsPaginatedFilterByNameOpt(Pageable pageable, UUID playerId, String name);

    @Query(value = SHEET_PAGINATED_BASE_SELECT + " from SheetModel s where s.isActive = true and lower(s.name) like lower(concat('%', ?1,'%'))",
            countQuery = "select count(*) from SheetModel s where s.isActive = true and lower(s.name) like lower(concat('%', ?1,'%'))")
    Page<BasicSheetDto> findActiveSheetsPaginatedFilterByNameOpt(Pageable pageable, String name);

    @Query(value = SHEET_PAGINATED_BASE_SELECT + " from SheetModel s where lower(s.name) like lower(concat('%', ?1,'%'))",
